...
export const componentDatabaseV2 = {
    ...
    compatibilityRules: {
      ...
      checkRearDerailleur: function(rd, cassette, crankset) {
        ...
        if (maxCog > rd.maxCog) {
          ...
          // Suggest alternative
          const compatibleRDs = componentDatabaseV2.rearDerailleurs.filter(
            altRD => altRD.maxCog >= maxCog && 
            altRD.speeds === rd.speeds && 
            altRD.bikeType === rd.bikeType
          );
          
          if (compatibleRDs.length > 0) {
            recommendations.push(`Consider upgrading to ${compatibleRDs[0].model} which supports ${compatibleRDs[0].maxCog}T`);
          }
        }
        ...
        if (crankset.teeth.length > 1) {
          ...
          if (requiredCapacity > rd.totalCapacity) {
            ...
            recommendations.push('Consider a longer cage derailleur or reduce gear range');
          }
        }
        ...
        if (rdSpeed !== cassetteSpeed) {
          ...
          const compatibleCassettes = componentDatabaseV2.cassettes.filter(
            c => c.speeds === rd.speeds && c.bikeType === cassette.bikeType
          );
          if (compatibleCassettes.length > 0) {
            recommendations.push(`Try ${compatibleCassettes[0].model} for ${rd.speeds} compatibility`);
          }
        }
  
        // Electronic/wireless compatibility
        if (rd.isElectronic && !rd.isWireless) {
          warnings.push('Electronic shifting requires proper cable routing and Di2 battery');
        }
        
        if (rd.isWireless) {
          warnings.push('Wireless shifting requires battery maintenance and proper pairing');
        }
  
        // Direct mount compatibility (SRAM T-Type)
        if (rd.isDirectMount) {
          warnings.push('Direct mount requires compatible frame with UDH (Universal Derailleur Hanger)');
        }
  
        // Brand-specific warnings
        const rdBrand = rd.model.split(' ')[0].toLowerCase();
        const cassetteBrand = cassette.model.split(' ')[0].toLowerCase();
        const cranksetBrand = crankset.model.split(' ')[0].toLowerCase();
        
        if (rdBrand !== cassetteBrand || rdBrand !== cranksetBrand) {
          // Known compatibility exceptions
          const knownCompatible = [
            { rd: 'shimano', cassette: 'sram', note: 'Works with proper shifter adjustment' },
            { rd: 'sram', cassette: 'shimano', note: 'May require cassette spacer' }
          ];
          
          const compatibility = knownCompatible.find(
            comp => comp.rd === rdBrand && comp.cassette === cassetteBrand
          );
          
          if (compatibility) {
            warnings.push(`Cross-brand setup: ${compatibility.note}`);
          } else {
            warnings.push('Mixed brands may require careful setup and tuning');
          }
        }
      ...
      // Enhanced chain length calculation
      calculateChainLength: function(crankset, cassette, chainstayLength = 430, hasClutch = false) {
        const bigRing = Math.max(...crankset.teeth);
        const bigCog = Math.max(...cassette.teeth);
        
        // More accurate formula accounting for derailleur cage
        const baseLength = (2 * chainstayLength / 12.7) + (bigRing + bigCog) / 2 + 2;
        
        // Add extra links for clutch derailleurs
        const finalLength = hasClutch ? baseLength + 2 : baseLength;
        
        return {
          links: Math.ceil(finalLength),
          recommendation: `Approximately ${Math.ceil(finalLength)} links needed`,
          notes: hasClutch ? 'Added 2 links for clutch derailleur tension' : 'Standard calculation'
        };
      },
  
      // New: Check if components work together as a system
      validateDrivetrain: function(crankset, cassette, rearDerailleur) {
        const rdCheck = this.checkRearDerailleur(rearDerailleur, cassette, crankset);
        const chainLength = this.calculateChainLength(crankset, cassette, 430, rearDerailleur.hasClutch);
        
        // Check for common issues
        const systemIssues = [];
        
        // 1x vs 2x compatibility
        if (crankset.teeth.length === 1 && !rearDerailleur.model.includes('1x')) {
          systemIssues.push('Single chainring with non-1x derailleur may have chain line issues');
        }
        
        // Gravel mullet setups
        if (cassette.bikeType === 'mtb' && crankset.bikeType === 'gravel') {
          systemIssues.push('MTB cassette with gravel crankset - verify chain line compatibility');
        }
        
        // Electronic system completeness
        if (rearDerailleur.isElectronic) {
          systemIssues.push('Electronic setup requires compatible shifters, battery, and wiring');
        }
      ...
      // New: Assess setup difficulty
      assessSetupDifficulty: function(crankset, cassette, rearDerailleur) {
        let difficulty = 'easy';
        let factors = [];
        
        // Electronic systems are more complex
        if (rearDerailleur.isElectronic) {
          difficulty = 'moderate';
          factors.push('Electronic shifting setup');
        }
        
        // Wireless adds complexity
        if (rearDerailleur.isWireless) {
          difficulty = 'moderate';
          factors.push('Wireless pairing required');
        }
        
        // Cross-brand setups
        const brands = [
          crankset.model.split(' ')[0],
          cassette.model.split(' ')[0],
          rearDerailleur.model.split(' ')[0]
        ];
        if (new Set(brands).size > 1) {
          difficulty = difficulty === 'easy' ? 'moderate' : 'hard';
          factors.push('Mixed brand compatibility');
        }
        
        // High-end components
        if (rearDerailleur.model.includes('XTR') || rearDerailleur.model.includes('Dura-Ace')) {
          factors.push('Precision adjustment required');
        }
        
        return { level: difficulty, factors };
      }
    }
  };
  ...
  // Enhanced compatibility rules for lib/v2/components.js
  export const enhancedCompatibilityRules = {
    // Original checkRearDerailleur function enhanced
    checkRearDerailleur: function(rd, cassette, crankset) {
      const errors = [];
      const warnings = [];
      const recommendations = [];
